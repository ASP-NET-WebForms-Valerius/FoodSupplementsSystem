...
namespace FoodSupplementsSystem.Web
{
    public class SeedDataAlexDb
    {
        ...
        private ICommentsServices CommentsServices { get; set; }
        ...
        {
            this.Categories.Add(new Category() { Name = "Vitamins" });
            this.Categories.Add(new Category() { Name = "Minerals" });
            this.Categories.Add(new Category() { Name = "Amino acids" });
            this.Categories.Add(new Category() { Name = "Digestive Enzimes" });
            this.Categories.Add(new Category() { Name = "Vitamin C 100% Powder" });
            this.Categories.Add(new Category() { Name = "Herbal Extracts" });
            this.Categories.Add(new Category() { Name = "Detox and Cleanse" });
        }
        private void GenerateToplics()
        {
            // 0
        ...
        private void GenerateSupplements()
        {
            this.Supplements.Add(new Supplement()
            {
        ...
        private void InsertCategories()
        {
            ...
            {
                this.UnitOfWork.CategoryRepository.Add(item);
            }

            this.UnitOfWork.CategoryRepository.SaveChanges();
        }
        private void InsertToplics()
        {
            foreach (Topic item in this.Topics)
            {
                this.UnitOfWork.TopicRepository.Add(item);
            }

            this.UnitOfWork.TopicRepository.SaveChanges();
        }
        private void InsertBrands()
        {
            foreach (Brand item in this.Brands)
            {
                this.UnitOfWork.BrandRepository.Add(item);
            }

            this.UnitOfWork.BrandRepository.SaveChanges();
        }
        private void InsertSupplements()
        {
            foreach (Supplement item in this.Supplements)
            {
                this.UnitOfWork.SupplementRepository.Add(item);
            }

            //this.UnitOfWork.SupplementRepository.SaveChanges();
        }
        private void InsertComments()
        {
            foreach (Comment item in this.Comments)
            {
                this.CommentsServices.Create(item);
            }
        }
    }
}
